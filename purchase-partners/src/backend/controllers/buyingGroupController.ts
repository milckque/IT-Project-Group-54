...
export const getBuyingGroups = async (_req: Request, res: Response) => {
...
export const joinBuyingGroup = async (req: Request, res: Response) => {
  ...
  const { error } = await supabase.from("BuyingGroupMembers").insert([
    {
      group_id: group_id,
      buyer_id: profile.id,
    },
  ...
  return res
    .status(201)
    .json({ success: true, message: "joined buying group" });
};
